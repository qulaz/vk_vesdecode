...
const Basket = ({ match: { params: { areaId, itemId }}, foodAreas, order }) => {
  const currentOrderState = JSON.parse(localStorage.getItem("currentOrder") || "{}")

  const [ faster, setFaster ] = useState(currentOrderState.faster || true);
  const [ time, setTime ] = useState(currentOrderState.time || '');
  const [ selfService, setSelfService ] = useState(currentOrderState.selfService || false);
    ...
    <div className="Place">
      ...
      <div className="Place__products-wrapper">
        ...
        <Link
          ...
          onClick={() => {
            const currentOrder = { faster, time, selfService }
